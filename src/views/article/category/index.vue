  ...
  <div class="content-info">
    <el-form ref="listFormRef" :inline="true" :model="formData" class="list-form">
      ...
      <el-form-item label="分类状态" prop="status">
        <el-select v-model="formData.status" placeholder="请选择分类状态" clearable>
    ...
    <category-drawer :isShow="categoryDrawerInfo.isShow"
                     :title="categoryDrawerInfo.title"
                     :type="categoryDrawerInfo.type"
                     :id="categoryDrawerInfo.id"
                     @close="categoryDrawerClose" />
  </div>
</template>
...
import categoryDrawer from './components/edit.vue'
...
const categoryDrawerInfo = reactive({
  isShow: false,
  title: '新增',
  type: 1,
  id: null
})
const categoryDrawerClose = val => {
  categoryDrawerInfo.isShow = false
  if (val) {
    getList(query)
  }
}
const add = () => {
  categoryDrawerInfo.isShow = true
  categoryDrawerInfo.type = 1
  categoryDrawerInfo.id = ''
  categoryDrawerInfo.title = '分类新增'
}
const edit = row => {
  categoryDrawerInfo.isShow = true
  categoryDrawerInfo.type = 2
  categoryDrawerInfo.id = row.id
  categoryDrawerInfo.title = '分类修改'
}
...
const delData = async () => {
  if (multipleSelection.value.length < 1) {
    return ElMessage.error('请选择需要删除的分类')
  }
